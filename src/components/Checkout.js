...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    { 
      try{ let response=await axios.post(`${config.endpoint}/cart/checkout`,
      {"addressId":addresses.selected},
      { headers: { Authorization: `Bearer ${token}`, }, })
       if(response.data.success){ enqueueSnackbar("Order placed successfully",{variant:"success"})
        let balance=localStorage.getItem("balance")
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              <Box className={`address-item ${addresses.selected===item._id? "selected":"not-selected"}`}
               ...
               <Typography> {item.address} 
               </Typography> 
               <Button className="explore-button" variant="text" onClick={()=>deleteAddress(token,item._id)}> 
               <Delete/> DELETE 
               </Button> </Box> ) : 
               <Typography my="1rem">No addresses found for this account. Please add one to proceed 
               </Typography>} 
               </Box>
            ...
            newAddress={newAddress}
             ...
             addAddress={addAddress} /> :
              ...
              size="large"
               ...
               ({ ...currNewAddress, isAddingNewAddress: true, })); }} > 
